...
public class NeighbourFragmentFav extends Fragment implements DeleteNeighbour {
    ...
    private void initList() {
        ...
        List<Neighbour> newList = new ArrayList<>();

        for(int i = 0; i< neighbours.size(); i++) {
            if (neighbours.get(i).getFav()) {
                newList.add(neighbours.get(i));
            }
        }

        mBinding.listNeighbours.setAdapter(new MyNeighbourRecyclerViewAdapter(newList, this));
    }
    ...
    public void clickOnTrash(Neighbour neighbour) {
        Toast.makeText(getContext(), getResources().getString(R.string.remove_this_neighbour_favorites_list), Toast.LENGTH_SHORT).show();
